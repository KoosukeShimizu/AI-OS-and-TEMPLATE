...
- 69_figma_design_with_nds.md  
  → use_figma MCP で NDS_v2 を使って Figma デザインを生成・配置する場合に参照（コーディング前のデザイン工程）
...
- @69_figma_design_with_nds.md — NDS_v2 でデザイン生成するときのトークン選択・use_figma 手順
...
| 69 | **Figmaデザイン生成規約** | use_figma MCP で NDS_v2 を使ってデザインを作るときのトークン選択・Surface/Text/Brand の使い分け・生成手順。デザイン工程（コーディング前）に参照 |
...
実作業を通じて新しいルール・思想が生まれた場合は以下のフローで AI-OS 本体に反映する。

### キャプチャ（実作業中）

`_ai-os/03_os_inbox.md` に以下の形式でメモを残す（その場で書く、形式は自由）：
...
> `_ai-os/03_os_inbox.md` の未処理メモをレビューして、AI-OSの各ファイルに統合してください。
> 既存ルールと矛盾する場合は統合前に確認してください。統合後はステータスを「統合済み」に更新してください。

### 2ステップの区別

| ステップ | 誰が起点 | 何をする |
|---|---|---|
| **インボックス追加** | AI が検知 → ユーザー承認 | `03_os_inbox.md` に未処理メモを追記 |
| **正式 OS アップデート** | ユーザーが明示依頼 | インボックスを各 OS ファイルへ統合 |

### 注意

- AI は `03_os_inbox.md` を自律的にマージしない（人間が明示的に依頼するまで待つ）
- 既存ルールと矛盾する場合は必ず人間に確認してからマージする
- 正式アップデートのタイミングの目安：未処理3件以上 / 大きなプロジェクトの節目