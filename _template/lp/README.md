...
## Design System 連携（AI-OS 準拠）

このプロジェクトは AI-OS フレームワークのデザインシステム運用規約に準拠しています。

DS 関連のドキュメント・設定は `design-system/` 配下に集約されています：

- `design-system/README.md` — DS全体概要
- `design-system/decisions.md` — 意思決定ログ
- `design-system/figma-reference.md` — Figmaファイル参照
- `design-system/tokens/tokens.json` — トークンデータ（W3C DTCG形式）

運用ルールの詳細は `_ai-os/60_figma_variables.md` 〜 `67_design_system_workflow.md` を参照してください。

上記 SCSS トークンシステムは、Figma Variables から生成される形に移行することが推奨されます（`Figma → tokens.json → Style Dictionary → tokens.scss`）。詳細は `_ai-os/63_design_tokens_output.md` を参照。
