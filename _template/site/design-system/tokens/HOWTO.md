...
### 大規模 tokens.json の取り扱い

変数数が 500 を超える、または JSON が 200KB を超える場合、AI のコンテキスト制約で一括変換が難しくなる。
カテゴリ別（color / spacing / typography / effect 等）に分割して順次変換し、差分はカテゴリ単位で `decisions.md` に記録する。
